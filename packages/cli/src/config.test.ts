...
const original = process.env.IBIRDUI_REGISTRY_URL;

afterEach(() => {
  if (original === undefined) Reflect.deleteProperty(process.env, 'IBIRDUI_REGISTRY_URL');
  else process.env.IBIRDUI_REGISTRY_URL = original;
});

describe('resolveRegistry', () => {
  it('prefers the explicit flag', () => {
    process.env.IBIRDUI_REGISTRY_URL = 'https://env.dev';
  ...
  it('falls back to the built-in default', () => {
    Reflect.deleteProperty(process.env, 'IBIRDUI_REGISTRY_URL');
