...
> The thesis: shadcn gave you **ownership**. everstate adds back the two things
...
`everstate add` writes a `everstate.lock.json` recording the version and a fingerprint of
every file. `everstate upgrade` then does a safe, edit-aware update:
...
- Every item carries a machine-readable **manifest** (`intents`, `examples`),
  aggregated into `r/manifest.json`. `everstate gen "<prompt>"` matches it to suggest
...
packages/cli       add · upgrade · gen · list  (writes/maintains everstate.lock.json)
...
node "$CLI" add     data-list --registry "$REG" --cwd /tmp/app   # also writes everstate.lock.json
node "$CLI" upgrade --registry "$REG" --cwd /tmp/app
```

The default registry URL is GitHub Pages; `--registry` / `EVERSTATE_REGISTRY_URL`
override it.
...
`pnpm registry:build` validates each item against `everstate-core`, injects a content
