...
> The thesis: shadcn gave you **ownership**. liveui adds back the two things
...
`liveui add` writes a `liveui.lock.json` recording the version and a fingerprint of
every file. `liveui upgrade` then does a safe, edit-aware update:
...
- Every item carries a machine-readable **manifest** (`intents`, `examples`),
  aggregated into `r/manifest.json`. `liveui gen "<prompt>"` matches it to suggest
...
packages/cli       add · upgrade · gen · list  (writes/maintains liveui.lock.json)
...
node "$CLI" add     data-list --registry "$REG" --cwd /tmp/app   # also writes liveui.lock.json
node "$CLI" upgrade --registry "$REG" --cwd /tmp/app
```

The default registry URL is GitHub Pages; `--registry` / `LIVEUI_REGISTRY_URL`
override it.
...
`pnpm registry:build` validates each item against `@liveui/core`, injects a content
