...
> The thesis: shadcn gave you **ownership**. ibirdui adds back the two things
...
`ibirdui add` writes a `ibirdui.lock.json` recording the version and a fingerprint of
every file. `ibirdui upgrade` then does a safe, edit-aware update:
...
`pnpm registry:build` validates each item against `ibirdui-core`, injects a content
