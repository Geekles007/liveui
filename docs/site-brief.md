# Build the marketing + docs website for **ibirdui**

You are building the official website for **ibirdui**, an open-source React component library. The site must be production-quality, fully accessible, and itself a showcase of the library's philosophy. Output a complete, runnable project.

## 1. What ibirdui is (context — use these facts, do not invent)

ibirdui is a **registry-as-code** component library: like shadcn/ui, you don't install a runtime dependency — a CLI copies the component's source **into your repo**, so you own the code. ibirdui's differentiator over shadcn:

> **The thesis:** shadcn gave you *ownership*. ibirdui adds back the two things ownership cost you — *completeness* (components handle every real-world state) and *maintainability* (a real upgrade path + AI-readiness).
...
**2. Upgradeable copy-paste.** `ibirdui add` writes a `ibirdui.lock.json` (version + a content fingerprint per file). `ibirdui upgrade` then updates files you haven't touched, and for files you edited locally it **does not overwrite** — it keeps yours and drops the new version as `*.new` to merge. Show this with an **animated terminal**:
...
**3. Accessible & AI-native.** Every UI component ships an **axe accessibility test** and documents its guarantees (live-region announcements, `aria-busy`, focus management, correct roles) — accessibility is *verified*, not claimed. Every component also carries a machine-readable **manifest** (`intents`, `examples`) so `ibirdui gen "<prompt>"` can suggest the right components for a task. Show an **AI prompt box**: user types "a list of users with loading states", it returns suggested components (`data-list`, `state-boundary`) with the `ibirdui add` command. (Backed by static data is fine.)
...
1. **Hero** — name, the one-line thesis, two CTAs: a primary copy-to-clipboard pill showing `npx ibirdui add data-list`, and a "Star on GitHub" button. Visual centerpiece: a small live component cycling through the 5 states (with a manual override). A subtle "pulse / vital-signs" motif fits the "live" brand.
...
4. **Component gallery** — a filterable grid (filter by layer and by status). Each card: name, plain-English description, states it handles (chips), a11y badge (AA), and the `ibirdui add <name>` command with copy button. Done components are interactive previews; planned ones show a "Planned" badge.
5. **How it works** — a clear diagram: `registry/items/*` → static JSON (`/r/*.json`) → `ibirdui` CLI copies files into your project + writes `ibirdui.lock.json`. Explain "no backend, just static files."
6. **Comparison table** — ibirdui vs shadcn/ui vs hand-rolled, rows: *You own the code*, *Loading/empty/error built-in*, *Optimistic & offline states*, *Verified accessibility (axe)*, *Upgrade path*, *AI manifest*. (ibirdui = all yes.)
7. **Quick start** — copyable, with package-manager tabs (npm / pnpm / bun). Show `npx ibirdui list`, `npx ibirdui add data-list`, `npx ibirdui upgrade`.
...
- **Dark mode first**, with a light theme and an accessible toggle. **Dogfood ibirdui's own theme tokens** (semantic CSS variables: `background`, `foreground`, `muted`, `border`, `destructive`, `primary`).
...
- All interactive demos announce state changes via a polite live region (mirroring ibirdui's own `state-boundary`).
- Images/icons have proper alt/aria; copy buttons have accessible labels and success feedback.

## 7. Technical constraints

- **Next.js (App Router) + TypeScript + Tailwind CSS.** Must support **static export** (deployed to GitHub Pages under base path `/ibirdui`) — no server runtime, no database.
...
**Brand voice:** confident, precise, developer-to-developer. No fluff, no buzzword soup. Lead with what breaks today and how ibirdui fixes it. Sample hero copy: *"Components that handle the parts you always forget. Own the code like shadcn — but with every state, verified accessibility, and an upgrade path that survives your edits."*
